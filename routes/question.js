...
const imgFolder = `http://localhost:5000/images/files/`; // change to process.env.PORT
questionRouter.route('/').get((req, res) => {
    Movie.aggregate([{ $sample: { size: 4 } }])
      ...
      .then(movies => {
          ...
          const imageURL = imgFolder + movie.images[Math.floor(Math.random() * movie.images.length)].name;
